...
bool navigation_test(std::string& subtest)
{
    ...
    if(!compare(coord.get_lat(), 5.0f, 0.00001f))
        return false;
    if(!compare(coord.get_lng(), 6.0f, 0.00001f))
        return false;

    subtest = "Coordinate from vector";
    Vector<2> v(3.6, -146.3);
    coord = Coordinate(v);
    if(!compare(coord.get_lat(), 3.6f, 0.00001f))
        return false;
    if(!compare(coord.get_lng(), -146.3f, 0.00001f))
        return false;

    subtest = "Coordinate from 3d vector";
    Vector<3> v3(3.6, -146.3, 90);
    coord = Coordinate(v3);
    if(!compare(coord.get_lat(), 3.6f, 0.0001f))
        return false;
    if(!compare(coord.get_lng(), -146.3f, 0.00001f))
        return false;
    ...
    v = coord;
    if(!compare(v.x(), 35.63f, 0.0001f))
        return false;
    if(!compare(v.y(), -147.63f, 0.000001f))
        return false;

    subtest = "Bearing to";
    coord = Coordinate(35.0f, 53.0f);

    if(!compare(coord.bearing_to(Coordinate(35, 54)), 89.7132f, 0.00001f))
        return false;

    subtest = "Distance to";
    coord = Coordinate(22.755920f, 3.515625f);
    if(!compare(coord.distance_to(Coordinate(21.779905, 7.382812f)), 412458.0f, 0.5f))
        return false;

    subtest = "Distance to";
    coord = Coordinate(20.303318f, 23.026157f);
    if(!compare(coord.distance_to(Coordinate(20.303307f, 23.026180f)), 2.66f, 0.1f))
        return false;
